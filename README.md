...
使用 OpenAI 官方 API 时，只需设置 API Key：
...
使用其他 LLM 服务商时，还需指定 Base URL 和 Provider：
...
docker run -d \
  ...
  -e UPSTREAM_PROVIDER=deepseek \
  ghcr.io/jiulinuoyi/codex-rosetta:latest
```

> **环境变量说明：**
> - `UPSTREAM_API_KEY` — 上游服务商的 API Key（必填）
> - `UPSTREAM_BASE_URL` — 上游 API 地址（默认 `https://api.openai.com/v1`）
> - `UPSTREAM_PROVIDER` — 上游服务商标识（默认 `openai`），常见值：`openai`、`deepseek`、`openrouter` 等

> **注意：** 此方式不含 Redis，会话状态存储在内存中，容器重启后会丢失。生产环境建议使用下方 Docker Compose 方式。
